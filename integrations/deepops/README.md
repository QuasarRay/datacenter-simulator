...
Each benchmark requests one task and one real GPU per compute VM. Tests use doubles, 256 B–1 MiB messages, five timed iterations, and **correctness enabled** (`-c 1`), with all reductions/roots where relevant. Reports must cover every requested size/operation/root combination and both execution modes. Upstream does not check in-place correctness for `alltoall`, `alltoallv`, or `sendrecv`; their `nwrong: null` fields remain explicitly unchecked. Reports must identify every distinct compute hostname and MPI rank, the expected native NCCL version, nonempty checked results, zero wrong values, no errors, and the required Socket/fabric environment. Slurm's `--export=NIL` keeps provisioning-machine environment variables out of guest jobs and upstream JSON environment dumps.
