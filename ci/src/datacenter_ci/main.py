...
class DatacenterCi:
    def runner(self, source: dagger.Directory, run_id: str) -> dagger.Container:
        ...
        return (dag.container().from_('ubuntu@sha256:224a1869083a311ef3f13648a154ba79832fbef6364d31493642ca03082da254')
                .with_exec(['bash','-ec','apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends ca-certificates docker.io iproute2 iputils-ping iptables openssh-client openssh-server jq python3 python3-pip python3-venv sudo git && rm -rf /var/lib/apt/lists/*'])
                ...
                .with_exec(['python','cheatsheets/ex457/v6/tools/patch_libssh.py','/opt/collections'])
