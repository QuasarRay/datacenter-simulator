...
pub fn loopback(payload:&[u8], timeout:Duration)->Result<Vec<u8>> {
    if payload.is_empty() || payload.len()>1024*1024 || timeout.is_zero() { bail!("payload must be 1 byte..1 MiB and timeout must be positive"); }
    let devices=ibverbs::devices()?;
    let device=devices.iter().next().context("no RDMA device; configure SoftRoCE or attach an RNIC")?;
    let ctx=device.open()?;
    let cq=ctx.create_cq(16).build()?;
    let pd=ctx.alloc_pd()?;
    let gid=ctx.routable_gid(1)?.context("no routable GID on RDMA port 1")?.gid_index;
    let prepared=pd.create_qp::<ibverbs::Rc>(&cq,&cq,1)?.set_gid_index(gid).build()?;
    ...
    unsafe { batch.submit() }?;
    let deadline=Instant::now().checked_add(timeout).context("timeout overflow")?;
    let mut sent=false; let mut received=false;
    while !sent || !received {
        {
        ...
        if Instant::now()>=deadline { bail!("RDMA completion timeout"); }
        std::thread::yield_now();
    }
    let result=mr.bytes_mut()[..payload.len()].to_vec();
    if result!=payload { bail!("RDMA payload mismatch"); }
    Ok(result)
}
