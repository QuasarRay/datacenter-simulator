use datacenter_simulator::{Simulator,manifest::Manifest,linux::LinuxFabric};
...
async fn run()->anyhow::Result<()> {
    ...
    let id=api.import(Manifest::from_json(include_str!("gpu-spine-leaf.json"))?,true)?;
    let sim=api.get(&id)?;
    let a=sim.node_named("gpu1")?.id.clone(); let b=sim.node_named("gpu2")?.id.clone();
    ...
    let server=linux.device(&b)?.spawn(move |_|async move {
        ...
        ready_tx.send(()).map_err(|_|anyhow::anyhow!("client disappeared"))?;
        let (mut socket,_)=listener.accept().await?;
        let mut payload=[0u8;17]; socket.read_exact(&mut payload).await?;
    ...
    tokio::time::timeout(std::time::Duration::from_secs(10),async { client.await??;server.await??;anyhow::Ok(()) }).await??;
    linux.set_link_up(&first,false).await?;
    anyhow::ensure!(linux.ping(&a,&b).await.is_err(),"partition unexpectedly has a route");
