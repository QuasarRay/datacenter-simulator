...
from fabric import intent, model, ping_ok, require
STATE = ROOT / '.state'
...
def prepare(public_key):
    ...
    for name, node in sorted(DC['nodes'].items()):
        folder = STATE / 'hostkeys' / name; folder.mkdir(parents=True, mode=0o700, exist_ok=True)
...
def preflight():
    ...
    require('0.79.0' in version, 'this lifecycle contract requires Containerlab 0.79.0')
    image = json.loads(run(['docker', 'image', 'inspect', 'ex457-frr:v5'], privileged=True).stdout)
...
def wait_ready():
    for name in DC['nodes']:
        for attempt in range(60):
            r = run(['docker', 'exec', 'clab-ex457-v5-' + name, 'vtysh', '-c', 'show version'], privileged=True, check=False)
            if r.returncode == 0:
                break
            time.sleep(1)
        else:
...
def dataplane():
    ...
    for name in DC['nodes']:
        ...
        for remote in desired['remote_nodes']:
            ...
            require(bool(rows), 'kernel route missing: ' + prefix)
            dst = str(ipaddress.IPv4Network(prefix))
            def valid(row):
                observed = row.get('dst', '')
                if '/' not in observed:
                    observed += '/32'
                gateways = [row.get('gateway')] + [n.get('gateway') for n in row.get('nexthops', [])]
                return observed == dst and any(g in desired['peers'] for g in gateways) and row.get('type', 'unicast') == 'unicast'
            require(any(valid(r) for r in rows), 'kernel prefix/nexthop mismatch: ' + prefix)
    ...
    out = STATE / 'dataplane.json'; out.write_text(json.dumps(transcript, indent=2) + '\n')
    print(f'{len(transcript)} sourced-ping/kernel-route pairs passed; {out}')
...
def main():
    ...
    if args.action == 'prepare':
        ...
        require('@sha256:' in args.base, 'supply an inspected containerlab-flavor FRR base digest')
        STATE.mkdir(mode=0o700, exist_ok=True)
        build = run(['docker', 'build', '--build-arg', 'FRR_BASE=' + args.base, '-t', 'ex457-frr:v5', ROOT / 'lab/frr'], privileged=True)
        (STATE / 'image-build.log').write_text(build.stdout + build.stderr)
        (STATE / 'image-inspect.json').write_text(run(['docker', 'image', 'inspect', 'ex457-frr:v5'], privileged=True).stdout)
        ...
        if args.action == 'restart':
            ...
            reloads = []
            for name in DC['nodes']:
                loaded = run(['docker', 'exec', 'clab-ex457-v5-' + name,
