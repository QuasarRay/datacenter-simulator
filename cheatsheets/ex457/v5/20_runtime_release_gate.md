...
For R1 inspect with `sudo docker exec clab-ex457-v5-spine1 passwd -S ansible` and `sudo docker exec clab-ex457-v5-spine1 sshd -T`. Do not publish `/etc/shadow` or private key material. For R2 make a temporary public trust file for one host with a different key; use `EX457_KNOWN_HOSTS` only for the negative run. Restore the verified file for recovery.
...
`CLOSED_RUNTIME`: reserved for an actual reviewed live transcript. No v5 row uses this label yet.
