...
```bash
ansible-playbook playbooks/backup_fabric.yml -e backup_run_id=before-drift
...
The backup play requires an HTTPS `backup_url` and `EX457_BACKUP_TOKEN` for Controller jobs. It PUTs each file to `<base>/<job-id>/<host>.cfg` with `If-None-Match: *`, GETs it back and compares exact bytes, then publishes `manifest.json` last. The service must enforce conditional creation; redirects and insecure TLS are rejected. Provide a service supporting this PUT/GET object contract and the issuing token; storage provisioning is an operator prerequisite. A failed export fails the backup job and blocks the workflow's Change node. Never count an ephemeral `/runner` file as a durable backup.

To recover, set `EX457_BACKUP_TOKEN` securely and run `python tools/backup_store.py download --run-id JOB_ID --url https://YOUR_SERVICE/ex457`, then run the restore play with `-e restore_run_id=JOB_ID`. The downloader reserves a fresh private directory and verifies the complete index, model hash and every file before recovery. An existing run ID is rejected. Keep credentials outside Git.
...
Backup captures use mode 0700 directories and 0600 files, including a durable checksum index. Reusing any run ID fails without overwriting prior bytes. Restore verifies the complete capture before configuring a router. The local persistence play requires Docker-host access; it is deliberately rejected inside Controller jobs.
