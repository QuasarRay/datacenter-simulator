...
from contracts import topology_ok, inventory_ok, effective_inventory_ok
from evidence_contract import manifest_ok, claims_ok, history_ok
...
def check_structure():
    ...
    claims_ok(claims, sources)
    history_ok(json.loads((ROOT/'evidence/audit-history.json').read_text()))
...
def main():
    ...
    try:
        result('structure_links_claims_and_blueprint',check_structure(),{'objectives':23,'families':8,'available_historical_findings':103})
    ...
    try:
        ...
        topology_ok(topology, dc, ROOT)
        inventory_ok(yaml.safe_load((ROOT/'inventory/network.yml').read_text()), dc)
        inventory_process = subprocess.run(['ansible-inventory', '--list', '-i', 'inventory/network.yml'], cwd=ROOT, env=env, text=True, capture_output=True, check=True)
        effective_inventory_ok(json.loads(inventory_process.stdout), dc)
    ...
    command('predicate_unit_tests',[sys.executable,'-m','unittest','discover','-s','tests','-p','test_fabric.py','-v'])
    if args.full:
